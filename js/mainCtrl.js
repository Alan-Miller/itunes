angular.module('itunes').controller('mainCtrl', function($scope, itunesService){
  ...
  $scope.gridOptions = {
      ...
      columnDefs: [
        ...
        {field: 'Explicitness', displayName: 'Collection Price'},
      ]
  };
  ...
  //Also note that that method should be retuning a promise, so you could use .then in this function.
    ...
    $scope.getSongData = function (artist) {
      itunesService.getSongData(artist).then(function(response) {  // ???
        // $scope.data = response.data.results[7].artistId;
        var dataResults = response.data.results;
        getDetails(dataResults);
  ...
  //is sort the data you got back to be an object in the following format.
    /*
      ...
      Type: "song"
  */
  //the iTunes API is going to give you a lot more details than ng-grid wants. Create a new array and then loop through the iTunes data pushing into your new array
  // objects that look like the above data.

    //Code here
    //

    function getDetails(dataResults) {
      var dataArray = [];
      for (i = 0; i < dataResults.length; i++) {
          dataArray.push({
            AlbumArt: dataResults[i].artworkUrl100,
            Artist: dataResults[i].artistName,
            Collection: dataResults[i].collectionName,
            CollectionPrice: dataResults[i].collectionPrice,
            Play: dataResults[i].previewUrl,
            Type: dataResults[i].trackName,
            Explicitness: dataResults[i].trackExplicitness
          });
      }
      $scope.songData = dataArray;
      if (dataResults[i].artistName === 'Kenny Ortega') {
        $scope.itsKenny = true;
      }
